...
def timed_broadcast(input, start_event, end_event, args, compute_tensors=None):
    if args.dist == 'torch':
    ...
    for i in range(args.warmups):
        if args.compute_overlap and compute_tensors is not None:
            handle = dist.broadcast(input, 0, async_op=True)
    ...
    if args.compute_overlap and compute_tensors is not None:
        ...
        for _ in range(args.trials):
            dist.broadcast(input, 0, async_op=False)
        end.record()
        sync_all()
        ...
        for M in M_LIST:
            ...
            try:
                ...
                if 'out of memory' in str(e):
                    ...
                    raise e
            sync_all()
            timed_broadcast(input, start_event, end_event, args, compute_tensors)
    else:
