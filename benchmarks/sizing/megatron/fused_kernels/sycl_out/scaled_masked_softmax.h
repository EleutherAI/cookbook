#define DPCT_COMPAT_RT_VERSION 10010
/* coding=utf-8
...
template <>
__inline__ void copy_vector<c10::BFloat16, 1>(c10::BFloat16* dst,
...
template <>
__inline__ void copy_vector<c10::BFloat16, 4>(c10::BFloat16* dst,
                                                         const c10::BFloat16* src)
{
...
template <>
__inline__ void copy_vector<c10::Half, 1>(c10::Half* dst, const c10::Half* src)
{
    *dst = *src;
}

template <>
__inline__ void copy_vector<c10::Half, 4>(c10::Half* dst, const c10::Half* src)
{
    *((float2*)dst) = *((float2*)src);
}
...
template <typename T>
struct Add {
    __dpct_inline__ T operator()(T a, T b) const { return a + b; }
};

template <typename T>
struct Max {
    __dpct_inline__ T operator()(T a, T b) const { return a < b ? b : a; }
};

template <typename T>
__dpct_inline__ T WARP_SHFL_XOR_NATIVE(T value, int laneMask, int width = 0,
                                       unsigned int mask = 0xffffffff)
{
#if DPCT_COMPAT_RT_VERSION >= 9000
    if (!width) width = sycl::ext::oneapi::this_work_item::get_sub_group()
                            .get_local_range()
                            .get(0);
...
__dpct_inline__ void warp_reduce(acc_t *sum)
{
    ReduceOp<acc_t> r;
#pragma unroll
...
void scaled_masked_softmax_warp_forward(output_t* dst,
                                                   const input_t* src,
                                                   const uint8_t* mask,
                                                   const acc_t scale,
                                                    ...
                                                    int element_count)
{
    ...
    // blockDim/threadIdx = (WARP_SIZE, WARPS_PER_BLOCK, )
    // gridDim/blockIdx = (seq_len, attn_heads, batches)
    int first_batch = (item_ct1.get_local_range(1) * item_ct1.get_group(2) +
                       item_ct1.get_local_id(1)) *
                      WARP_BATCH;
                                            ...
                                            int pad_batches)
{
    if (key_seq_len == 0) {
        ...
        int warps_per_block = (threads_per_block / warp_size);
        int batches_per_block = warps_per_block * batches_per_warp;
        dpct::dim3 blocks(query_seq_len / batches_per_block, attn_heads,
                          batches);
        dpct::dim3 threads(warp_size, warps_per_block, 1);
